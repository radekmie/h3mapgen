...
def make_grav_points(gap, xlim=(-2, 2), ylim=(-2, 2), grid=False):
    ...
    if grid:
        ...
        grid = np.concatenate([xx[:, :, np.newaxis], yy[:, :, np.newaxis]],
                              axis=2).reshape(-1, 2)
        return grid
...
def make_index(graph):
    temp = list(
        enumerate(sorted(graph.keys(), key=lambda x: tuple(x.split('_')))))
...
def plot_a_thing(data, graph, inds, vor, vor_zone_graph, cons, figname=None,
                 to_file=True, xlim=(-2, 2), ylim=(-2, 2), threshold=.5):
    ...
    for u_num in range(data.shape[0]):
        plt.text(data[u_num][0], data[u_num][1], inds[
                 0][u_num], color="red", fontsize=8)
        u = inds[0][u_num]
        ...
        if np.all(simplex >= 0):
            ...
            if not same_vert(inds[0][a], inds[0][b]):
                plt.plot(vor.vertices[simplex, 0],
                         vor.vertices[simplex, 1], 'k--')
        i += 1

    center = data.mean(axis=0)
    for pointidx, simplex_idx in zip(vor.ridge_points,
                                     range(len(vor.ridge_vertices))):
        simplex = np.asarray(vor.ridge_vertices[simplex_idx])
        a, b = vor.ridge_points[simplex_idx]
        if np.any(simplex < 0) and not same_vert(inds[0][a], inds[0][b]):
            ...
            far_point = vor.vertices[
                i] + np.sign(np.dot(midpoint - center, norm)) * norm * 100
            plt.plot([vor.vertices[i, 0], far_point[0]], [
                     vor.vertices[i, 1], far_point[1]], 'k--')

    plt.text(xlim[1]-4, ylim[1]+.1, 'Bad edges: ' +
             str(bad_edges), color="red", fontsize=20)
    plt.text(xlim[1]-2, ylim[1]+.1,
             'Inconsistent zones: ' + str(len(cons) - sum(cons.values())),
    ...
    grid = np.concatenate(
        [xx[:, :, np.newaxis], yy[:, :, np.newaxis]], axis=2).reshape(-1, 2)
...
def prepare_voronoi(graph, sizes, data):
    ...
    for key, value in vor_graph.items():
        vor_zone_graph[vert_id(key)] |= {
            vert_id(v) for v in value if vert_id(v) != vert_id(key)}
...
def zone_consistency(vor_graph, sizes):
    ...
    def accessible_same_id(wid):
        ...
        while acc_new:
            acc |= acc_new
            acc_new = {v for u in acc_new for v in vor_graph[
                u] if vert_id(v) == wid} - acc
...
def calc_bad_edges(graph, vor_zone_graph):
    ...
    for u in graph:
        for v in graph[u]:
            if (not same_vert(v, u) and
                    vert_id(v) not in vor_zone_graph[vert_id(u)]):
                bad_edges += 1
    return bad_edges / 2
...
if __name__ == '__main__':
    ...
    def embed():
        data_trans, E = sammon(
            dists, inputdist='distance', init='random', display=0)
        ...
        for i in range(10):
            ...
            data_trans_scaled = pull_points(
                data_trans_scaled, grav_points, mass, .5)

        vor, vor_graph, vor_zone_graph = prepare_voronoi(
            graph, sizes, data_trans_scaled)
        cons = zone_consistency(vor_graph, sizes)
        bad_zones = len(cons) - sum(cons.values())
        bad_edges = calc_bad_edges(graph, vor_zone_graph)

        return (data_trans_scaled, vor, vor_zone_graph, cons,
                E, bad_edges, bad_zones)
